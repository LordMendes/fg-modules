...
Replace the legacy scraper in [`fg-builder/`](../fg-builder/) (which targeted `dnd.arkalseif.info`) with a new Python scraper that downloads **all** D&D 3.5 reference data from [https://new.dndtools.org/](https://new.dndtools.org/) and writes normalized JSON files under `data/dndtools/`.
...
Create a new scraper at the `dndtools-reference/` project root (do not extend `fg-builder/scraper/`):
...
Reference `fg-builder/scraper/` for proven patterns (`http_client.py`, `engine.py`, `writer.py`, parser structure) but **rewrite parsers** for the new HTML structure.

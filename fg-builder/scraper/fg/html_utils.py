...
_LOOSE_TR_BLOCK_RE = re.compile(r"(?:<tr\b[^>]*>.*?</tr>\s*)+", re.I | re.S)
...
def prepare_formatted_html(html: str) -> str:
    ...
    inner = re.sub(r"\s*\n\s*", " ", inner)
    inner = sanitize_xml_text(inner.strip())
    return normalize_all_tables_in_html(inner)
...
def normalize_class_body_html(html: str) -> str:
    ...
    inner = normalize_all_tables_in_html(inner)
...
def wrap_loose_table_rows(html: str) -> str:
    """Wrap orphan <tr> sequences (outside existing <table> blocks) in <table>."""
    if not html or "<tr" not in html.lower():
        return html

    parts = _TABLE_BLOCK_RE.split(html)
    out: list[str] = []
    for i, part in enumerate(parts):
        if i % 2 == 1:
            ...
            out.append(
                _LOOSE_TR_BLOCK_RE.sub(lambda m: f"<table>{m.group(0)}</table>", part)
            )
    return "".join(out)


def normalize_all_tables_in_html(html: str) -> str:
    """Wrap loose rows and convert <th> headers to FG-safe <td><b>…</b></td>."""
    if not html or ("<table" not in html.lower() and "<tr" not in html.lower()):
        return html
    return normalize_fg_tables_in_html(wrap_loose_table_rows(html))
